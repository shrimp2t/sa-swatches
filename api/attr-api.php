...
namespace SASW_SWATCHES\API\Attrs;

use function SASW_SWATCHES\get_wc_tax_attrs;
use function SASW_SWATCHES\remove_empty_from_array;
use function SASW_SWATCHES\get_image_data;
use function SASW_SWATCHES\get_swatch_data;

add_action('sasw_api/update_term_swatch', __NAMESPACE__ . '\rest_update_term_swatch');
add_action('sasw_api/update_custom_swatch', __NAMESPACE__ . '\rest_update_custom_swatch');
add_action('sasw_api/get_terms', __NAMESPACE__ . '\rest_get_tax_terms');
add_action('sasw_api/get_attr_settings', __NAMESPACE__ . '\rest_get_attr_settings');
add_action('sasw_api/add_term', __NAMESPACE__ . '\rest_add_term');
add_action('sasw_api/get_product_attrs', __NAMESPACE__ . '\rest_get_product_attrs');
...
function get_terms_data($terms, $type = null, $pid = null, $tax = null, $image_size = 'thumbnail')
{
	...
	$list = [];
	$overwrite_all =  $pid ? get_post_meta($pid, '_sasw_custom_swatches', true) : [];
	...
	foreach ($terms as $term) {
		...
		if (isset($overwrite[$item_data['id']]) && $overwrite[$item_data['id']]) {
			...
			if (isset($custom['swatch'])) {
				...
				if ($custom['swatch']['type'] === 'sasw_image' && isset($custom['swatch']['value'])) {
...
function rest_get_attr_settings($post)
{
	...
	$data =  get_post_meta($pid, '_sasw_attr_settings', true);
	if (!is_array($data)) {
		$data = [];
	}
	wp_send_json([
...
function get_product_attributes($product)
{
	...
	$attrs = get_wc_tax_attrs();
	$attr_data = [];
	$all_settings =  get_post_meta($product->get_id(), '_sasw_attr_settings', true);
	...
	foreach ($attributes as $attribute_name => $attr_options) {
		...
		$attr_data[$key]['selected'] = isset($_REQUEST[$attr_data[$key]['name']]) ? wc_clean(wp_unslash($_REQUEST[$attr_data[$key]['name']])) : $attr_data[$key]['default']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
{
	$pid = isset($_GET['pid']) ? sanitize_text_field($_GET['pid']) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
